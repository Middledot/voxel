...
use crate::raknet::objects::{MsgBuffer, datatypes::to_address_bytes};
...
impl Serialize for OnlineConnAccepted {
    fn serialize(&self) -> MsgBuffer {
        ...
        buf.write_i64_be_bytes(&(SystemTime::now().duration_since(UNIX_EPOCH).expect("Oops").as_millis() as i64));
